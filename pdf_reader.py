...
def iterative_segment_image(image, max_segments=10, density_threshold=FORMULA_THRESHOLD):
    ...
    while segment_count < max_segments and kernel_size > 26 and formula_density > density_threshold:
        ...
        kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (305, kernel_size+100))
                ...
                if st.button(f"Process Page {i+1}"):
                    with st.spinner(f"Processing page {i+1}..."):
                        ...
                        for seg_path in SEGMENT_IMAGE_SET:
                            classification = classify_image(seg_path, use_easyocr=True)
